...
How the shared calibration code in this repo works. Operator bring-up is [`CALIBRATION_PROCEDURE.md`](CALIBRATION_PROCEDURE.md). Include/shim layout is in the [README](../README.md). Runtime amp-comp after calibration depends on each sketch’s `ENGINE_OPTIONS.md`.
...
Pin: `DCO_calibration_pin` (DCO3 **GP6**, DCO4 **GP10**). Feed the signal into the pin shown in `[CAL_SENSE] pin=`. Setup uses `pinMode` on that GPIO. Manual cal runs at `manual_DCO_calibration_start_note` (24) in step 0 and `manual_cal_reference_note` (69, **A4 = 440 Hz**) in step 1; PW calibration uses note 24. RP2040/RP2350 at 3.3 V IO: **VIH ≥ 2.0 V**, **VIL ≤ 0.8 V**.
